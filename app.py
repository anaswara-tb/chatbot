...
async def register(user: UserRegister):
    ...
    query = users_table.insert().values(name=user.name, email=user.email, password=hashed)
...
async def upload_dataset(project_id: int, file: UploadFile = File(...), payload: dict = Depends(verify_token)):
    proj = await database.fetch_one(
        projects_table.select().where(
            (projects_table.c.id == project_id) & (projects_table.c.user_id == payload["user_id"])
        )
    )
    if not proj:
...
for idx, record in enumerate(data):
    text = record.get('text', '').strip()
        ...
        indent = record.get('intent', '').strip() if 'intent' in record else None
...
async def preview_dataset(dataset_id: int, limit: int = 50, payload: dict = Depends(verify_token)):
    ds = await database.fetch_one(datasets_table.select().where(datasets_table.c.id == dataset_id))
    if not ds:
...
async def predict_entities(request: PredictRequest, payload: dict = Depends(verify_token)):
...
async def get_annotations(dataset_id: int, skip: int = 0, limit: int = 10, payload: dict = Depends(verify_token)):
...
async def save_annotation(request: SaveAnnotationRequest, payload: dict = Depends(verify_token)):
    """Save user annotation"""
    update_query = annotations_table.update().where(
        annotations_table.c.id == request.record_id
    ).values(
...
async def export_annotations(dataset_id: int, format: str = "json", payload: dict = Depends(verify_token)):
